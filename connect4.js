 ...
 */
 ...
 function makeHtmlBoard() {
     ...
     let elems = document.querySelector("#row-top");
     [...elems.childNodes].forEach(function (elem) {
         let posit = elem.getBoundingClientRect();
         let ind = elem.getAttribute('id');
         const tWidth = posit.right - posit.left;
         elem.style.left = ind * tWidth + 'px';
     });
 
     let playRows = document.querySelectorAll("#play-row");
     [...playRows].forEach(function (rows) {
         [...rows.childNodes].forEach(function (playEl) {
             let posit = playEl.getBoundingClientRect();
             let ind = playEl.getAttribute('id');// ind is an array of 3 elements: 'y', '-', 'x'
             const tWidth = posit.right - posit.left;         
             const tHeight = posit.bottom - posit.top;
             playEl.style.left = ind[2] * tWidth + 'px';
             playEl.style.top = tHeight + ind[0] * tHeight + 'px';
         });
     });
 }
 ...
 function placeInTable(y, x) {
     ...
     ball.setAttribute("id", `B${y}`);// added id to offset animation drop in css
 ...
 function endGame(msg) {
     document.querySelector("#row-top").removeEventListener('click',handleClick);
     alert(msg);
 }
